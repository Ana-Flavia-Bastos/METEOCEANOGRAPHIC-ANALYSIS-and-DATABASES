...
#var_list = 'surf_el,water_temp,salinity,water_u,water_v',wnd10mx0.5 (vento)
var='ocnslh'

north = -18.5     # -80 to 80 degN          
south = -19.5  # -80 to 80 degN
west = -39   # -180 to 180 degE
east = -38    # -180 to 180 degE

#Grid Lat=North/South (90 to -90) Lon=West/East (-180 to 180)
...
#year_inicial=input('Provide the First year of data (Years available: 1979 - 2023): ')
#year_final=input('Provide the Last year of data (Years available: 1979 - 2023): ')
date_start = '1979-01-01 01:00:00'
date_final = '2023-01-01 00:00:00'
...
ThreddsUrlv1 = 'https://thredds.rda.ucar.edu/thredds/ncss/grid/files/g/ds093.1/'
ThreddsUrlv2 = 'https://thredds.rda.ucar.edu/thredds/ncss/grid/files/g/ds094.1/'         # include the ending '/'

################################## Functions #####################################

# Check the folders created
def create_folders_downloaded():
    path='./Wind_Data_Downloaded_CFSR_'+project+'/'
...
def correct_longitude_degree(degree):
    if degree<0:
        degree_corrected = degree+359.5 # 0 to 360 degE
    elif degree==0:
...
def get_extraction_wind(dt_i,dt_f,out_fn):
    ...
    if int(year)<2011:
        url =(ThreddsUrlv1+str(year)+'/wnd10mx0.5.gdas.'+str(year)+str(month)+'.grb2'+
    ...
    elif year=='2011' and month in ["01","02","03"]:
        url =(ThreddsUrlv2+str(year)+'/wnd10mx0.5.gdas.'+str(year)+str(month)+'.grb2'+
        '?var=u-component_of_wind_height_above_ground&var=v-component_of_wind_height_above_ground'+
        '&north='+str(north)+'&west='+str(west2)+'&east='+str(east2) +'&south='+str(south)+
        '&horizStride=1&'+
        '&time_start='+dstr0+'&time_end='+dstr1 +
    ...
    elif year =='2011' and month in ["04","05","06","07","08","09","10","11","12"]:
        url =(ThreddsUrlv2+str(year)+'/wnd10mx0.5.cdas1.'+str(year)+str(month)+'.grb2'+
        '?var=u-component_of_wind_height_above_ground&var=v-component_of_wind_height_above_ground'+
        '&north='+str(north)+'&west='+str(west2)+'&east='+str(east2) +'&south='+str(south)+
        '&horizStride=1&'+
        '&time_start='+dstr0+'&time_end='+dstr1+
...
def get_extraction_seasurfaceheight(dt_i,dt_f,out_fn):
    dstr0 = dt_i.strftime('%Y-%m-%d-T%H:00:00Z')
    dstr1 = dt_f.strftime('%Y-%m-%d-T%H:00:00Z')
    year=dt_i.strftime('%Y')
    month=dt_i.strftime('%m')
    ...
    if int(year)<2011:
        url =(ThreddsUrlv1+str(year)+'/ocnslh.gdas.'+str(year)+str(month)+'.grb2'+
        '?var=Sea_Surface_Height_Relative_to_Geoid_surface_1_Hour_Average'+
        '&north='+str(north)+'&west='+str(west2)+'&east='+str(east2) +'&south='+str(south)+
        '&horizStride=1&'+
        '&time_start='+dstr0+'&time_end='+dstr1 +
        '&&&accept=netcdf4-classic&addLatLon=true')
        ...
        url =(ThreddsUrlv2+str(year)+'/ocnslh.gdas.'+str(year)+str(month)+'.grb2'+
        '?var=Sea_Surface_Height_Relative_to_Geoid_surface_1_Hour_Average'+
        '&north='+str(north)+'&west='+str(west2)+'&east='+str(east2) +'&south='+str(south)+
        '&horizStride=1&'+
        '&time_start='+dstr0+'&time_end='+dstr1 +
        ...
        url =(ThreddsUrlv2+str(year)+'/ocnslh.cdas1.'+str(year)+str(month)+'.grb2'+
        '?var=Sea_Surface_Height_Relative_to_Geoid_surface_1_Hour_Average'+
        '&north='+str(north)+'&west='+str(west2)+'&east='+str(east2) +'&south='+str(south)+
        '&horizStride=1&'+
        '&time_start='+dstr0+'&time_end='+dstr1+
...
if int(DateF.strftime('%Y'))==2023:
    nyt=(int(DateF.strftime('%Y'))+1-int(DateI.strftime('%Y')))
elif int(today.strftime('%Y'))==2024:
    nyt=1
else:
...
print('\n** Working on dowload of CFSR complete **')
...
for dt in range(len(dt_ilist)):
    ...
    if not os.path.isfile(out_fn):
        if var=='wnd10mx0.5':
            result = get_extraction_wind(dt_ilist[dt], dt_flist[dt], out_fn)
        else:
            result = get_extraction_seasurfaceheight(dt_ilist[dt], dt_flist[dt], out_fn)
