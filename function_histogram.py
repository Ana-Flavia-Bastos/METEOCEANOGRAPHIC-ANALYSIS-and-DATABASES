...
def probability_histogram(df,path_results,variables,bins, time, point, depth, histogram_data):
    ...
    if variables in ['Water Level','Hm0']:
    ...
    ax_cumulative.set_ylabel('Cumulative [%]', color='black')
    ax.set_title(point+' '+depth+' \n '+f'Histogram - {variables} - {time}')
    ...
    print('Create ' +time+' ' +variables+' Histogram ')
    fig.savefig(path_results+point+'_Histogram_'+variables +'_'+time+'.png',bbox_inches='tight')
