...
def organize_bins(variables,bins):
    ...
    if variables not in ['Direção do Vento','Direção da Corrente','Direção média de Ondas']:
...
def plot_heat_map(cross_table, x_axis_name, y_axis_name, period,path_results,time,point,depth):
    ...
    fig.suptitle(point+' '+depth+'\nTabela de Frequência - '+x_axis_name+' X '+y_axis_name+' - '+time)
    plt.rc('font', size=9)
    # Main heat map
    ax0 = plt.subplot(gs[0])
    plot_table = cross_table.iloc[0:-2,0:-2]
    if x_axis_name in ['Velocidade do Vento','Velocidade da Corrente']:
        plot_table.index.name = x_axis_name + ' [m/s]'  
    elif x_axis_name in ['Altura Significativa']:
        plot_table.index.name = x_axis_name + ' [m]'  
    elif x_axis_name in ['Período']:
        plot_table.index.name = x_axis_name + ' [s]'

    if y_axis_name in ['Velocidade do Vento','Velocidade da Corrente']:
        plot_table.columns.name = y_axis_name + ' [m/s]'
    elif y_axis_name in['Altura Significativa']:
        plot_table.columns.name = y_axis_name + ' [m]'
    elif y_axis_name in ['Direção do Vento','Direção média de Ondas']:
        plot_table.columns.name = y_axis_name + ' [°N-de]'
    elif  y_axis_name in ['Direção da Corrente']:
        plot_table.columns.name = y_axis_name + ' [°N-para]'
    elif y_axis_name in ['Período']:
    ...
    # Save to file
    plt.tight_layout()
    fig.savefig(path_results+point+'_TabelaFrequencia_'+x_axis_name+' vs. '+y_axis_name+'_'+time+'.png')
...
def plot_rose(dfs_intervals,nsector,variables,bins,time,path_results,point,depth):
    ...
    if len(variables)==3:
        ...
        for v in range(len(variables)):
            if variables[v] in ['Velocidade do Vento','Velocidade da Corrente']:
                name_columnVar.append(variables[v])
                bins_l.append(bins[v])
                title.append(variables[v] + ' [m/s]')
            elif variables[v] in ['Altura Significativa']:
                name_columnVar.append(variables[v])
                bins_l.append(bins[v])
                title.append(variables[v] + ' [m]')
            elif variables[v] in ['Período']:
                name_columnVar.append(variables[v])
        ...
        for n in range(len(name_columnVar)):
            ...
            for i in range(len(dfs_intervals)):
                ...
                direction_column=df.loc[:]['Direção média de Ondas']
                # Plot rose
                ...
                if name_columnVar[n] in ['Velocidade do Vento']:
                    plt.title(point+' '+depth+'\nRosa dos Ventos - '+name_columnVar[n]+' - '+time[i])
                elif name_columnVar[n] in ['Velocidade da Corrente']:
                    plt.title(point+' '+depth+'\nRosa das Correntes - '+name_columnVar[n]+' - '+time[i])
                elif name_columnVar[n] in ['Altura Significativa']:
                    plt.title(point+' '+depth+'\nRosa das Ondas - '+name_columnVar[n]+' - '+time[i])

                print('Criando Rosa '+time[i]+' da variavel '+n)
                fig.savefig(path_results+point+'_PlotRosa_'+name_columnVar[n]+' vs. '+name_dir+'_'+time[i]+'.png',bbox_inches='tight',pad_inches=0.1)
        ...
        for i in range(len(dfs_intervals)):
            ...
            fig.text(0.5, 0.5, '%', color='black',
                bbox=dict(facecolor='black', edgecolor='black', boxstyle='circle', color='lightgray'))
            if n in ['Velocidade do Vento']:
                plt.title(point+' '+depth+'\nRosa dos Ventos - '+n+' - '+time[i])
            elif n in ['Velocidade da Corrente']:
                plt.title(point+' '+depth+'\nRosa das Correntes - '+n+' - '+time[i])
            elif n in ['Altura Significativa']:
                plt.title(point+' '+depth+'\nRosa das Ondas - '+n+' - '+time[i])
            

            print('Criando Rosa '+time[i]+' da variavel '+n)
            fig.savefig(path_results+point+'_PlotRosa_'+n+' vs. '+n_dir+'_'+time[i]+'.png',bbox_inches='tight',pad_inches=0.1)
