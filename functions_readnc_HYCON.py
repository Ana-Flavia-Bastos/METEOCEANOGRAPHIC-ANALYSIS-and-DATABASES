...
def dfseries(nc,latitude_idx,longitude_idx,dtime_l):
    ...
    for t in range(len(dtime_l)):
        #wt_l.append(nc.variables['water_temp'][t,0,latitude_idx,longitude_idx])
        sal_l.append(nc.variables['salinity'][t,0,latitude_idx,longitude_idx])
        dt_l.append(dtime_l[t])

    #df_temp=pd.DataFrame(zip(dt_l,wt_l))
    df_temp=pd.DataFrame(zip(dt_l,sal_l))
    return df_temp
